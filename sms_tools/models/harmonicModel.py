...
def harmonicDetection(pfreq, pmag, pphase, f0, nH, hfreqp, fs, harmDevSlope=0.01):
    ...
    if not f0:  # if no f0 return no harmonics
        return np.zeros(nH), np.zeros(nH), np.zeros(nH)
    ...
    if hfreqp == []:  # if no incomming harmonic tracks initialize to harmonic series
        hfreqp = hf
    while (f0 > 0) and (hi < nH) and (hf[hi] < fs / 2) and (pfreq != []):  # find harmonic peaks
...
def harmonicModel(x, fs, w, N, t, nH, minf0, maxf0, f0et):
    ...
    while pin < pend:
        ...
        hfreqp = hfreq
        # -----synthesis-----
        Yh = UF.genSpecSines_p(hfreq, hmag, hphase, Ns, fs)  # generate spec sines
...
def harmonicModelAnal(x, f0, fs, w, N, H, t, nH, minf0, maxf0, f0et, harmDevSlope=0.01, minSineDur=.02):
    ...
    while pin <= pend:
        ...
        hfreq, hmag, hphase = harmonicDetection(ipfreq, ipmag, ipphase, f0[f0_idx], nH, hfreqp, fs,
